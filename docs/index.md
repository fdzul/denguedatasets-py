...
The easiest way to install the `denguedasets` package is directlty from PyPI:
...
```bash
pip install git+https://github.com/fdzul/denguedatasets_py
```
...
See all dataset included in the `denguedasets` package
...
df <- dd.list_datasets("dengue_brazil")
